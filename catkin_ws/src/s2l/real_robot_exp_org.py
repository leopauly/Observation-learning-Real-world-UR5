...
STD_DEV_ARR = np.array([5e-2, 1e-4, 1e-4, 1e-4, 2e-2, 1e-4]) # std_dev for exploration
TIMEOUT = 20000
MIN_ROLLOUT = 2
...
class real_robot_trajopt():
    ...
    def traj_opt(self):
        ...
        self.reset_robot()
        self.reset_object()
        sequences = np.zeros((MAX_ITERATIONS, self.cand_ctrl_seq.shape[0],
        ...
        plt.ion()
        while cand_reward <= REWARD_THRESH or max_iter <= MAX_ITERATIONS:
            reward_array = np.zeros(self.K)
